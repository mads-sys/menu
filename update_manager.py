...
def log(message, level="INFO"):
    ...
    # Avisos (warnings) são enviados para stderr para serem capturados pelo backend.
    if level == "WARN":
        print(f"W: {message}", file=sys.stderr)
    # Erros também vão para stderr.
    elif level == "ERROR":
        print(f"{message}", file=sys.stderr)
    # Mensagens de sucesso/info vão para stdout.
    else:
        print(message)
...
def run_command(command, env=None):
    ...
    try:
        result = subprocess.run(
            ...
            encoding='utf-8',
            env=env,
            # check=False é importante para que possamos inspecionar o resultado
            # manualmente em vez de levantar uma exceção imediatamente.
            check=False 
        )
        return result
...
def update_apt():
    ...
    log("Gerenciador de pacotes 'apt' detectado.")
    env = os.environ.copy()
    ...
    log("Sistema (APT) atualizado com sucesso.")
    return True
...
def update_dnf():
    ...
    if not result or result.returncode != 0:
        log(f"Falha ao executar 'dnf upgrade'. Detalhes: {result.stderr.strip() if result else 'Comando não encontrado'}", "ERROR")
        return False
...
def update_yum():
    ...
    if not result or result.returncode != 0:
        log(f"Falha ao executar 'yum update'. Detalhes: {result.stderr.strip() if result else 'Comando não encontrado'}", "ERROR")
        return False
...
def update_pacman():
    ...
    if os.path.exists("/var/lib/pacman/db.lck"):
        log("Lock do Pacman ('db.lck') encontrado. Outro processo pode estar em execução.", "ERROR")
        return False
    result = run_command(["pacman", "-Syu", "--noconfirm"])
    if not result or result.returncode != 0:
        log(f"Falha ao executar 'pacman -Syu'. Detalhes: {result.stderr.strip() if result else 'Comando não encontrado'}", "ERROR")
        return False
...
def main():
    ...
    if os.path.exists("/usr/bin/apt-get"):
        ...
        log("Nenhum gerenciador de pacotes suportado (apt, dnf, yum, pacman) foi encontrado.", "ERROR")
        sys.exit(1)
